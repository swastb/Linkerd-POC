...
public class HeaderClassifier extends Function<ReqRep, ResponseClass> {
    ...
    public boolean isDefinedAt(ReqRep reqRep) {
        Request req = (Request)reqRep.request();
        String tokenValue = req.headerMap().get("token").get();
        ...
        rep.headerMap().set("token",tokenValue);
    ...
    public ResponseClass apply(ReqRep reqRep) {
        ...
        switch (status) {
            case "success": System.out.println("Got success "); return ResponseClass$.MODULE$.Success();
